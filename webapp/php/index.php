    ...
    return new class($c) {
        ...
        public function db_initialize() {
            ...
            foreach($users as $user) {
                $user_posts = $db->prepare('SELECT `p`.`id`, `p`.`user_id`, `u`.`account_name` AS `user_account_name`, `body`, `mime`, `p`.`created_at` FROM `posts` AS `p` JOIN `users` AS `u` ON `u`.`id` = `p`.`user_id` WHERE `p`.`user_id` = ? AND `u`.`del_flg` = 0 ORDER BY `p`.`created_at` DESC LIMIT ' . POSTS_PER_PAGE)->execute([$user['id']])->fetchAll();
                $this->cache->set('user_post_'.$user['id'], json_encode($user_posts));
            }
...
$app->get('/@{account_name}', function (Request $request, Response $response, $args) {
    ...
    $results = json_decode($this->cache->get('user_post_'.$user['id']), true);
    ...
    if (!$results) {
        ...
        $ps->execute([$user['id']]);
        $results = $ps->fetchAll(PDO::FETCH_ASSOC);
        $this->cache->set('user_post_'.$user['id'], json_encode($results));
    }
